...
- 全屏 `cover` 铺底，界面容器半透明让壁纸透出；**无蒙雾遮罩**（中间背景保持壁纸原样，不铺主题纯色：主题色只作用于侧边栏/对话框/按钮等组件，背景底层为固定深色兜底 `#0a0c12`，删除原 `.ff-bg-shade` 全屏渐变遮罩层）
- 「**景**」按钮弹出常驻面板：
