			function addWallpaper() {
				...
				input.type = "file";
				// 不设 accept 媒体类型限制：iOS/Android 上 accept 只含图片/视频会直接弹「相册」，
				// 去掉限制后系统选择器出现「浏览/文件」入口，用户从「文件」App 选（原始文件名由文件系统正确给出）。
				// 文件类型仍由服务端 ALLOWED_UPLOAD_EXT 校验兜底（不支持返回 400，客户端计入 failed）。
				...
				input.addEventListener("change", async () => {
					...
					for (const file of files) {
						...
						// 上传到服务器磁盘（raw body + ?name= 原始文件名），成功后用服务器 URL。
						// 服务端落盘即原始文件名（同名不同内容自动加 (1)(2)… 后缀）；SHA-1 仅作内部去重。
