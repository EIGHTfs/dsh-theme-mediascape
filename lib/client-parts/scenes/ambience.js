// ═══════════ 6. 常驻萤火氛围（分档：关/星光/曳光/流萤，数量渐变）═══════════
		...
		// 2026-09-2x 拆（审计 max-function-length）：粒子生成与档位行提为纯 helper
		// 粒子 DOM 生成：随机尺寸/透明度/动画参数，返回 dots 数组（后续档位切换用）
		function createAmbientDots(wrap, maxCount) {
			const dots = [];
		...
		// 档位 UI 行：插到「景」面板（壁纸设置）标题行之后；面板缺失时隐藏该行（不误占 dock）
		function createAmbienceLine(dock, panel, onCycle) {
			...
			if (panel === dock) line.style.display = "none";
		...
		function startAmbience(dock) {
			...
			const dots = createAmbientDots(wrap, maxCount);
			...
			const cycleBtn = createAmbienceLine(dock, panel, () => {
