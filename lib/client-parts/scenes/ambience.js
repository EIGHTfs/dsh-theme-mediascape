// ═══════════ 6. 常驻萤火氛围（分档：关/星点/曳光/流萤，数量渐变）═══════════
		...
		// 氛围粒子容器仍挂在 body（画面级效果），档位 UI 由 startAmbience 插进 dock.__ffBgPanel。
		const AMB_LEVELS = [
		...
		function startAmbience(dock) {
			...
			line.className = "ff-bg-line ff-amb-line";
			const lab = document.createElement("span");
			lab.className = "ff-bg-label";
			lab.textContent = "氛围";
			...
			for (const lv of AMB_LEVELS) {
				...
				b.type = "button";
				b.className = "ff-bg-seg ff-amb-opt";
				b.dataset.key = lv.key;
				b.textContent = lv.label;
				b.addEventListener("click", () => { setLevel(lv.key); });
				line.appendChild(b);
				segs.push(b);
			}
			const panel = dock.__ffBgPanel || dock;
			...
			function setLevel(key, instant) {
				...
				segs.forEach((b) => b.classList.toggle("active", b.dataset.key === lv.key));
			}
