		...
		function startWallpaper(dock) {
			...
			bg.className = "ff-bg";
			// 蒙雾层 .ff-bg-shade 已移除：中间背景不铺主题色，壁纸原样透出（组件仍随主题换色）
			...
			function render(item) {
				...
				// 主题联动（自动配色）已废弃：不再随壁纸自动换配色（基底 --ff-theme-* 由一键应用/基底 css 固定）
				// 原调用 ffUpdateThemeFromWallpaper(item) 与取色模块 theme.js 均停用（源码保留备查）
				if (item.kind === "video") {
			...
			function removeSelected() {
				...
				for (const id of ids) {
					...
					if (!item) continue;
					if (item.custom) {
						// 删除服务器上的壁纸文件（item.data = /theme-mediascape-assets/wallpaper/<file>）
						const m = /\/theme-mediascape-assets\/wallpaper\/([^?]+)$/.exec(item.data || "");
						if (m) {
							fetch(WALLPAPER_API + "/wallpaper/" + m[1], { method: "DELETE" }).catch(() => {});
						}
					} else {
						hidden.add(id);
					}
			...
			async function loadCustomWallpapers(refresh) {
				try {
					const resp = await fetch(WALLPAPER_API + "/wallpaper/list");
					if (!resp.ok) return;
