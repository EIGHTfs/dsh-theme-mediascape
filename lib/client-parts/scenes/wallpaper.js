		...
		// 2026-09-23 拆（审计 max-function-length renderLayers 182 行）：
			...
			async function removeSelected() {
				...
				for (const id of ids) {
					...
					// 2026-09-2x：带上 file/size（isItemUsable 真实可用校验需要——缺字段会被滤掉导致删不掉）
				...
				try {
					...
					if (refresh) {
						...
						for (const r of (data.items || [])) {
							...
							const item = { id: r.id, kind: r.kind, data: r.url, label: r.label || r.id, custom: true, etag: r.etag || null, size: r.size ?? null, file: r.file };
			...
			window.__mediascapeDshAfterWallpaperUpload = (lastUp) => {
				...
				const item = arr.find((w) => w.id === lastUp.id);
				if (item) showByIndex(item.kind, arr.indexOf(item));
