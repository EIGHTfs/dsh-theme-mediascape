// ═══════════ 10. 字体大小调节（字按钮：会话区/深度求索等文字缩放）═══════════
		...
		function startFont(dock) {
			...
			function applyScale(scale) {
				...
				document.body.style.setProperty("--ff-font-size", size + "px");
				document.body.style.setProperty("--ff-font-line", line + "px");
				document.body.style.setProperty("--ff-font-line-status", lineStatus + "px");
			...
			return () => {
				...
				btn.remove();
				menu.remove();
				document.body.style.removeProperty("--ff-font-size");
				document.body.style.removeProperty("--ff-font-line");
				document.body.style.removeProperty("--ff-font-line-status");
