		...
		// 2026-09-2x 拆（审计 max-function-length）：按钮创建与档位循环提为纯 helper

		// dock「字」按钮创建（点击循环逻辑在 createFontCycle）
		function createFontToggle(dock) {
		...
		// 档位循环状态机：LEVELS + 当前档 + setLevel（applyScale + 按钮高亮）+ 点击循环 + 恢复 + 卸载
		function createFontCycle(btn, applyScale) {
			const LEVELS = [
		...
		function startFont(dock) {
			const btn = createFontToggle(dock);
			...
			return createFontCycle(btn, applyScale);
		}
