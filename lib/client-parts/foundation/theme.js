		...
		// 原设计：identity.js 的 html 选择器定义 --ff-theme-* 默认值（= 流萤原色）。
		// 这里只「覆盖」这些变量：图片壁纸切换时取主色 → 生成 --ff-theme-* 三元组覆盖，
		// 组件样式（identity.js）已统一 var() 引用，自动跟随换肤；视频不触发（沿用图片最后一套）。
		// 命名空间：--ff-theme-* 管「壁纸联动皮肤」，--dsw-alias-* 管「固定品牌色」，两者并存。
		...
		/**
		 * 从主色生成一套 --ff-theme-* 变量值（与 identity.js 基底变量名一一对应）。
		 * 输出 RGB 三元组字符串（"r, g, b"）——组件样式用 rgba(var(--ff-theme-x), a) 组合出各透明度层次，
		 * 换肤只改三元组，透明度层次自动跟随。
		 */
		function ffBuildTheme(colors) {
			...
			return {
				...
				"--ff-theme-bg": triple(ffHslToRgb(base.h, base.s * 0.4, 12)),
				"--ff-theme-bg-soft": triple(ffHslToRgb(base.h, base.s * 0.3, 18)),
				"--ff-theme-bg-layer": triple(ffHslToRgb(base.h, base.s * 0.25, 24)),
				// 强调族：高饱和（accent-dim 由组件透明度组合出淡底）
				"--ff-theme-accent": triple(ffHslToRgb(accent.h, Math.min(accent.s * 1.2, 80), 60)),
				"--ff-theme-accent-soft": triple(ffHslToRgb(accent.h, Math.min(accent.s * 0.8, 50), 40)),
				// 文字族：高亮度保证对比度
				"--ff-theme-text": triple(ffHslToRgb(base.h, 10, 92)),
				"--ff-theme-text-dim": triple(ffHslToRgb(base.h, 8, 65)),
				// 边框族
				"--ff-theme-border": triple(ffHslToRgb(base.h, base.s * 0.3, 32)),
			};
		}
