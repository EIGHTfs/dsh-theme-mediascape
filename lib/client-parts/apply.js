		...
		function apply(ctx) {
			ctx.effect(() => {
				...
				// 幂等：若因 DSH 热重载/重复 apply 导致旧悬浮层残留，先移除再新建，避免叠层
				document.querySelectorAll(".ff-dock, .ff-bg, .ff-bg-panel, .ff-bg-picker, .ff-ms-picker, .ff-music-card, .ff-snd-menu, .ff-font-menu").forEach((el) => el.remove());
				...
				const onEsc = (e) => {
					...
					document.querySelectorAll(".ff-bg-panel.open, .ff-bg-picker.open, .ff-ms-picker.open, .ff-font-menu.open").forEach((el) => el.classList.remove("open"));
