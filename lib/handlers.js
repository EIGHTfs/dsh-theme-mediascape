...
import { loadLabels, loadMusicLabels, saveLabels } from './labels.js';
import { MUSIC_LABELS_FILE, ONLINE_DIR_NAME, WALLPAPER_LABELS_FILE, musicDir, musicLabelsPath, onlineDir, wallpaperDir } from './paths.js';
...
export function handleDelete(req, res, rel) {
  try {
    ...
    // 仅允许 wallpaper/ 目录内的常规文件（排除 .trash-* 回收文件自身）
      ...
      const online = readdirSync(odir)
        ...
        .map((f) => {
          ...
          return {
            ...
            kind: ext === '.mp4' ? 'video' : 'image',
            label: labels[id] || f.replace(/\.[^.]+$/, ''),
            url: '/theme-mediascape-assets/wallpaper/' + ONLINE_DIR_NAME + '/' + encodeURIComponent(f),
          };
        });
      items.push(...online);
    }
    ...
    try {
      ...
      if (map[digest] !== undefined || existsSync(file)) {
        ...
        res.end(JSON.stringify({
          ...
          label,
          traceId,
          url: '/theme-mediascape-assets/wallpaper/' + encodeURIComponent(digest + ext),
        }));
        return;
      }
