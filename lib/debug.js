...
function parseDebugConfig(cfg) {
  if (!cfg || typeof cfg !== 'object') {
    return { log: false, themeSwatch: false, preview: false };
  }
  const result = {
  ...
  // 自动透传未知键：以后加 debug 开关只写 debug.json，本文件无需再改
...
export function readDebugConfig(force = false) {
  ...
  let cfgResult;
  try {
    cfgResult = parseDebugConfig(JSON.parse(readFileSync(debugConfigPath(), 'utf8')));
  } catch {
