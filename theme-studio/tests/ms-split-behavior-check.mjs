...
// ── 1) apply 注册链路 ──
// ⚠️ 不调 apply()：它会触发 ensureOnlineDownload() 发起真实网络下载，测试进程会因
// 未完成请求挂起（此前 exit 超时被杀）。注册链路只验证 registerAssets 即可覆盖路由。
const wired = registerAssets(fakeCtx);
...
mkdirSync(join(D, 'theme-mediascape', 'wallpaper'), { recursive: true });
writeFileSync(join(D, 'theme-mediascape', 'wallpaper', 'custom-demo123.mp4'), 'demo-content', 'utf8');
writeFileSync(join(D, 'theme-mediascape', 'wallpaper', 'wallpaper.json'), JSON.stringify({ 'custom-demo123': '演示壁纸' }), 'utf8');

// ── 3) GET wallpapers/list ──
{
  const res = makeRes();
  registeredHandler(makeReq('GET', '/theme-mediascape-assets/wallpaper/list'), res);
...
{
  const res = makeRes();
  registeredHandler(makeReq('DELETE', '/theme-mediascape-assets/wallpaper/custom-not-exist.mp4'), res);
  const ok = res.status === 400;
...
{
  const res = makeRes();
  registeredHandler(makeReq('DELETE', '/theme-mediascape-assets/wallpaper/custom-demo123.mp4'), res);
